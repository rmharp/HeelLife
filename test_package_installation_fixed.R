...
# Comprehensive HeelLife Package Installation and Testing Script (Fixed Version)
# This script handles cleanup, installation, and testing of the updated package
# Fixed to handle temporary directory issues more carefully

cat("🧹 HeelLife Package Installation and Testing Script (Fixed)\n")
cat("=========================================================\n\n")
...
# Clean up webdriver manager cache (but be more careful with temp dirs)
...
# Don't clean tempdir() while R is running - just clean specific webdriver files
cat("ℹ️  Skipping temporary directory cleanup while R is running\n\n")
...
if (!requireNamespace("devtools", quietly = TRUE)) {
  ...
  tryCatch({
    ...
    cat("❌ Error installing devtools:", e$message, "\n")
...
tryCatch({
  ...
  cat("❌ Error installing RSelenium:", e$message, "\n")
  cat("This may cause issues later. Trying to continue...\n\n")
})
...
tryCatch({
  # Force reload devtools to ensure it's working
  if (requireNamespace("devtools", quietly = TRUE)) {
    library(devtools)
  ...
  # Try alternative method - install from local source
  tryCatch({
    cat("🔄 Trying to install from local source...\n")
    devtools::install(".", force = TRUE)
    cat("✅ HeelLife package installed from local source!\n\n")
  }, error = function(e2) {
    cat("❌ Local installation also failed:", e2$message, "\n")
    cat("Cannot proceed without HeelLife package.\n\n")
    quit(status = 1)
  })
})
